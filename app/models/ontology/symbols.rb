module Ontology::Symbols
  ...
  module Methods
    def update_or_create_from_hash(hash, timestamp = Time.now)
      ...
      unless hash['name'] || hash['kind']
        Rails.logger.warn "Using work-around to determine symbol name and kind: #{e.inspect}"
  ...
  def delete_edges
    %i[parent_id child_id].each do |key|
      EEdge.where(key => self.symbols.where(kind: 'Class')).delete_all
    end
  end
    ...
    transaction requires_new: true do
      subclasses.each do |s|
        ...
        unless c1 == "Thing" || c2 == "Thing"
          child_id = self.symbols.where('name = ? OR iri = ?', c1, c1).first.id
          parent_id = self.symbols.where('name = ? OR iri = ?', c2, c2).first.id
