module Ontology::Links
  ...
  module Methods
    ...
    def update_or_create_from_hash(hash, user, timestamp = Time.now)
      ...
      source = Ontology.find_with_iri(source_iri) || (raise ArgumentError, "source #{Settings.OMS} not found: #{source_iri}")
      target = Ontology.find_with_iri(target_iri) || (raise ArgumentError, "target #{Settings.OMS} not found: #{target_iri}")
      ...
      link.save!
      link_version = LinkVersion.create(link: link,
