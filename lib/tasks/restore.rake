namespace :restore do
  ...
  task :ontology_version_paths => :environment do
    def restore_last_known_path(ontology_version)
      if prev_version = previous_version(ontology_version)
        if !prev_version.path.present?
          restore_last_known_path(prev_version)
        end
        restore_last_known_path_from_previous_version(ontology_version,
          prev_version)
      else
        restore_last_known_path_from_ontology(ontology_version)
      end
    end
    ...
    def restore_last_known_path_from_previous_version(version, prev_version)
    ...
    def restore_last_known_path_from_ontology(version)
      if !version.path.present?
